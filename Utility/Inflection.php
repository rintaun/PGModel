...
class Inflection {
    ...
    public static function camelize($str) {
        if (array_key_exists($str, static::$camelized)) {
            return static::$camelized[$str];
        }
        ...
        if (array_key_exists($camel, static::$decamelized)) {
            $f = __FUNCTION__;
            $e = static::$decamelized[$camel];

            throw new AmbiguousInflectionException($f, $str, $camel, $e);
        }

        static::$camelized[$str]     = $camel;
        static::$decamelized[$camel] = $str;
    ...
    public static function decamelize($str) {
        if (array_key_exists($str, static::$decamelized)) {
            return static::$decamelized[$str];
        }
        ...
        if (array_key_exists($out, static::$camelized)) {
            $f = __FUNCTION__;
            $e = static::$camelized[$out];

            throw new AmbiguousInflectionException($f, $str, $out, $e);
        }

        static::$decamelized[$str] = $out;
        static::$camelized[$out]   = $str;
    ...
    public static function pluralize($str) {
        ...
        if (preg_match('/(^|.*_)([^_]+)$/', $str, $m)) {
            if (in_array($m[2], static::$uncountable)) {
                return $str;
            }
            else if (array_key_exists($m[2], static::$irreg)) {
                return $m[1] . static::$irreg[$m[2]];
            }
            else {
                foreach (static::$suffixes as $sing => $plur) {
        ...
        if (preg_match('/(^|_)([^_]+)f(e?)$/', $str, $m)) {
            if ($m[3]) {
                $ret = static::_suf($str, $m[2], static::$fe_ves, 'fe',
                                  'ves');
            }
            else {
                $ret = static::_suf($str, $m[2], static::$f_ves, 'f', 'ves');
            }
        }
        else if (preg_match('/(^|_)([^_]+)o$/', $str, $m)) {
            $ret = static::_suf($str, $m[2], static::$o_oes, 'o', 'oes');
        }
        else if (preg_match('/(^|_)([^_]+)a$/', $str, $m)) {
            $ret = static::_suf($str, $m[2], static::$a_ae, 'a', 'ae');
        }
        else if (preg_match('/(^|_)([^_]+)us$/', $str, $m)) {
            $ret = static::_suf($str, $m[2], static::$us_i, 'us', 'i');
        }
        else if (preg_match('/(^|_)([^_]+)um$/', $str, $m)) {
            $ret = static::_suf($str, $m[2], static::$um_a, 'um', 'a');
        }
        else if (preg_match('/(^|_)([^_]+)is$/', $str, $m)) {
            $ret = static::_suf($str, $m[2], static::$is_es, 'is', 'es');
        }
        else if (preg_match('/(^|_)([^_]+)on$/', $str, $m)) {
            $ret = static::_suf($str, $m[2], static::$on_a, 'on', 'a');
        }
    ...
    public static function singularize($str) {
        ...
        if (preg_match('/(^|.*_)([^_]+)$/', $str, $m)) {
            if (in_array($m[2], static::$uncountable)) {
                ...
                $idx = array_search($m[2], static::$irreg);
                if ($idx !== false) {
                    return $m[1] . $idx;
                }
                else {
        ...
        if (preg_match('/(^|_)([^_]+)ves$/', $str, $m)) {
            $ret = static::_suf($str, $m[2], static::$fe_ves, 'ves', 'fe');
            if ($ret !== null) {
                return $ret;
            }

            $ret = static::_suf($str, $m[2], static::$f_ves, 'ves', 'f');
        }
        else if (preg_match('/(^|_)([^_]+)oes$/', $str, $m)) {
            $ret = static::_suf($str, $m[2], static::$o_oes, 'oes', 'o');
        }
        else if (preg_match('/(^|_)([^_]+)ae$/', $str, $m)) {
            $ret = static::_suf($str, $m[2], static::$a_ae, 'ae', 'a');
        }
        else if (preg_match('/(^|_)([^_]+)i$/', $str, $m)) {
            $ret = static::_suf($str, $m[2], static::$us_i, 'i', 'us');
        }
        else if (preg_match('/(^|_)([^_]+)a$/', $str, $m)) {
            $ret = static::_suf($str, $m[2], static::$um_a, 'a', 'um');
            if ($ret !== null) {
                return $ret;
            }

            $ret = static::_suf($str, $m[2], static::$on_a, 'a', 'on');
        }
        else if (preg_match('/(^|_)([^_]+)es$/', $str, $m)) {
            $ret = static::_suf($str, $m[2], static::$is_es, 'es', 'is');
        }
