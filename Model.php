...
use \PGModel\Database,
    ...
    \PGModel\Query\NoSuchRowException;
...
abstract class Model {
    ...
    function __construct($pkeys = null, $values = null) {
        ...
        $pkey_cols = static::primary_keys((string) get_class($this));
...
COLQUERY;
    ...
    private function _load_association($name, $force = false) {
        ...
        $mytable = static::table_for_class($myclass);
    ...
    public function column_names($class = null) {
        ...
        $keys = array_keys(static::columns($class));
        sort($keys);
    ...
    public static function primary_keys($class = null) {
        ...
        foreach (static::columns($class) AS $name => $col) {
            if ($col->primary_key()) {
    ...
    public function table() {
        return static::table_for_class((string) get_class($this));
    }
    ...
    public static function table_for_class($class) {
        if (!array_key_exists($class, self::$tbl_lookup)) {
            ...
            static::associate_table($table, $class);
        }
